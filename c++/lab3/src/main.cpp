...
#include "./worker-db/worker_db.hpp"
#include "./notification/notification.hpp"
#include "./bool-array/bool_array.hpp"
using namespace std;
...
int main() {
     ...
     */
    {
        //нельзя перегрузить *, т.к. перегруженные операторы методом класса на самом деле
        // вызывают m.operator*(double), то есть слева стоит this.
        // при double * m слева this нет и поэтому можно перегрузить только глобальной функцией
        Matrix m=(4);
        m = static_cast<double>(2) * m;
        m=-m;
        m.print();
    }
...

    {
        ...
        db["Petrov"] = WorkerData("Petr", 43, 1500.0);
        std::cout << "Ivanov's name = " << db["Ivanov"].get_name() << "\n";
        std::cout << "Petrov's age = " << static_cast<int>(db["Petrov"].get_age()) << "\n";
    } 
  ...
  /**
    ...
    {
        Notification n1 = create_system_notification(
            1,
            MyString("Системное уведомление"),
            Notification::Default
        );

        Notification n2 = create_system_notification(
            2,
            MyString("Системное уведомление (Критическое)"),
            Notification::Critical
        );

        Notification n3 = create_message_notification(
            3,
            MyString("Алексей"),
            MyString("Мгновенное уведомление")
        );

        Notification n4 = create_application_notification(
            4,
            MyString("Telegram"),
            MyString("Новое сообщение"),
            MyString("Уведомление от приложения")
        );
        ...
        cout << "count_notifications_by_type:\n";
        cout << "Системных: "
            << count_notifications_by_type(arr, 4, Notification::System) << '\n';
        cout << "Мгновенных: "
            << count_notifications_by_type(arr, 4, Notification::Message) << '\n';
        cout << "От приложения: "
            << count_notifications_by_type(arr, 4, Notification::Application) << '\n';
    }
    ...
    // Эту задачу можно было решить через наследование. Например, сделать базовый класс Notification с общими данными: timestamp, и виртуальным методом print(). 
    // А для каждого вида уведомления создать отдельный класс, который будет наследоваться от Notification. 
    // Тогда не нужно было бы использовать union, помнить и определять что в нем лежит

    // Подход с union экономит память. Но это сложнее в реализации и дольше по времени: 
    // нужно вручную следить за активным полем, вызывать конструкторы и деструкторы и тд. Легко ошибиться

    // Подход с наследованием проще и безопаснее с точки зрения логики. + низкий порог входа для нового разработчика
...

    {
        ...
        queue.push(create_system_notification(1, "system1", Notification::Default));
        queue.push(create_message_notification(2, "andrey1", "message1"));
        queue.push(create_application_notification(3, "app", "title", "body"));

        cout << "pop:\n";
        queue.pop().print(); 

        queue.push(create_system_notification(4, "system2", Notification::Critical));
        queue.push(create_message_notification(5, "andrey2", "message2"));

        for (Notification* it = queue.begin(); it != queue.end(); ++it) {
            it->print();
        }

        while (queue.get_size() > 0) {
            Notification current = queue.pop();
            current.print();
        }
    }
    ...
    {
        BaseFile file1("./public/base.txt", "rb+");
        file1.write("Hello file1", 11);

        cout << file1.can_read() << endl;
        
        BaseFile file2(std::move(file1));

        cout << file1.can_read() << endl;

        file2.write("Hello file2", 11);
        
        BaseFile file3("./public/base_rle.txt", "rb+");
        file3 = std::move(file2);

        file3.write("Hello file3", 11);
    }
