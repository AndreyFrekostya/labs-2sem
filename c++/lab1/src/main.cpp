...
int main()
{
    ...
    {  
        ...
        // компилятором автоматически создает экземпляр с мусорными значениями. 
        // В общем случае комплиятор создаёт конструктор и деструктор, которые:
        // 1) Конструктор: вызывает конструкторы для всех полей, если они сами классы
        // 2) Деструктор: вызывает деструкторы для всех полей, если они сами классы
    }
    ...
    {
        // Ошибка для создания rect выше: "для класса "Rect" не существует конструктор по умолчанию".
        // То есть при явном конструктуре компилятор не создает конструктор по умолчанию. Нужно вручную создать конструкто по умолчанию
...

    {
        ...
        Rect rect1(1, 2, 3, 4); //вызывается явно нами определённый конструктор с параметрами
        
        Rect rect2 = rect1; //компилятор создаёт неявный конструктор копирования и вызывает его:
                            //берёт каждое поле объекта rect1 и копирует его в rect2.
                            //с указателями копируются именно указатели, а не сами данные => переменные указывают на один и тот же адрес памяти
        
        Rect rect3(rect1);  //прямой вызов конструктора копирования, работает как и во 2 случае

    } // Здесь вызовутся деструкторы. 
     ...
     */
    {
        //Объекты, созданные внутри блока, будут существовать только до конца блока =>
        //Соответственно вызываются деструкторы при выходе из блочной области видимости
        //Деструкторы вызываются в обратном порядке снизу вверх. В нашем случае порядок десруктуров rect3 => rect2 => rect1
        //В общем случае деструктор используется для освобождения ресурсов и ручного освобождения памяти.
    }
     ...
     
    {
        ...
        Rect *pR = new Rect(1,2,1,2); // конструктор с параметрами  
        {
            ...
            Rect arRect[2]; //2 раза конструктор по умолчанию  
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i);// конструктор с параметрами вызывается один раз. Деструктор в конце main
                Rect r4(*pR); //конструктор копирования. 3 итерации => 3 вызова конструктора и деструктора
                Rect r5(i,i,i,i); // конструктор с параметрами. 3 итерации => 3 вызова конструктора и деструктора
            }// Деструкторы r4, r5 => 6 деструкторов всего для цикла
        }// Деструкторы для: r2, arRect[0], arRect[1] => 3 деструктора
        delete pR; // деструктора для указателя
    } // деструктор для r1
    ...
    {
        Rect r1;
        std::cout << r1.left << std::endl; // Поля c модификаторами доступа private доступны только внутри методов класса
    } 
    */
     ...
     */
    {
        //member-func всегда принадлежит объекту класса, имеет доступ к его скрытым данным и использует скрытый указатель this, 
        //тогда как обычная функция существует независимо от объектов класса и работает только с тем, что ей явно передано
    }
    ...
    {
        Rect r1(0, 5, 0, 3), r2(2, 7, 1, 4); // 2 конструктора с параметрами
        Rect r3 = bounding_rect(r1, r2); // 2 конструкторa копирования + 1 конструктор с параметрами для создания нового rect
        print_rect(r3);
    } 
...

    {
        ...
        cout << "Изначальная ширина: " << r.get_width() << "\n";    
        cout << "Изначальная высота: " << r.get_height() << "\n";  
        cout << "Изначальная площадь: " << r.get_square() << "\n";  

        r.set_width(15);   
        r.set_height(8);   

        cout << "Новая ширина: " << r.get_width() << "\n";         
        cout << "Новая высота: " << r.get_height() << "\n";       
        cout << "Новая площадь: " << r.get_square() << "\n";     
    }
    ...
    {
        Barrel alch(10, 9.6);  // 10л бочка, 9.6л спирта 
        Barrel water(10, 0);   // 10л бочка, 0л спирта; чистая вода

        int iteration = 0;

        while (alch.get_concentration() > 50.0) {
            alch.pour_into(water, 1.0);  // 1л переливается из alch в water
            water.pour_into(alch, 1.0);  // 1л переливается из water в alch
            iteration++;

            cout << "После итерации " << iteration << ":\n";
            alch.print();
            water.print();
        }

        cout << "Концентрация спирта в первой бочке < 50% после " << iteration << " итераций\n";
    } 
     ...
     */
    {
        //1) Использованы переменные
        //2) Использованы зависимости (normal и order-only)
        //3) Написан target clean
    }
    /**
